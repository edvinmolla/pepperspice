        ...
        if request.method == 'POST':
            ...
            if not UserTrait.objects.filter(email=request.user).exists():
                ...
                if DB_Details.linked_to_app:
                    ...
                    node.save()
                    print("Executing: echo toor | sudo -S docker run -d " + "--name " + DB_Details.db_uid + " --network=net1 -e MYSQL_ROOT_PASSWORD=toor" + " --ip 192.168.8.56"  + " mysql")
                    os.system("echo toor | sudo -S docker run -d " + "--name " + DB_Details.db_uid + " --network=net1 -e MYSQL_ROOT_PASSWORD=toor" + " --ip 192.168.8.56"  + " mysql")
                    time.sleep(5)
            ...
            #    The creation of mysql docker container and the omnidb container will be done here.
                    ...
                    os.system("echo toor | sudo -S docker run -d " + "--name '" + DB_Details.db_uid + "' -e MYSQL_ROOT_PASSWORD=KmQfcsnpdUrrRL2qzE8P --network=net1 --ip 192.168.8.56 mysql")
                    time.sleep(30)             
